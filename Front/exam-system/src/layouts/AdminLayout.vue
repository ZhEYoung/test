...
import { School, Monitor, Fold, Expand, ArrowDown, User, UserFilled, List, Reading, Document, Avatar } from '@element-plus/icons-vue'
...
const handleSaveProfile = async () => {
  ...
  try {
    ...
    const response = await axios.put(`/api/admin/admins/${profileInfo.value.adminId}`, {
...
const fetchProfileInfo = async () => {
  try {
    ...
    console.log('Fetching profile for userId:', userId) // 调试日志
    const response = await axios.get(`/api/admin/admins/${userId}`)
    console.log('Profile response:', response) // 调试日志

    if (response.data.code === 200 && response.data.data) {
      profileInfo.value = response.data.data
      console.log('Profile info updated:', profileInfo.value) // 调试日志
    } else {
